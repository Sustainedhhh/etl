...
public class SOAPWebService extends AbstractProcess {
		...
		for (int i = 0; i < node.getChildNodes().getLength(); i++) {
			...
			if (soapNode.getNodeType() == Node.ELEMENT_NODE) {
				if (soapNode.getNodeName().equals("Input")) {
					...
					soap.setAction(soapNode.getTextContent());
				} else if (soapNode.getNodeName().equals("URL")) {
					soap.setURL(soapNode.getTextContent());
				} else if (soapNode.getNodeName().equals("Process")) {
					soap.addProcess(Process.fill((soapNode)));
				}
			}
		}
		return soap;
	...
	public String toString() {
		return "SOAPWebservice ID[" + id + "], URL[" + URL + "], Input["
				+ input + "], Action[" + action + "], Input[" + input + "]";
	}
}
